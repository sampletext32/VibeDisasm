        ...
        for (var i = 0; i < bodyBlocks.Count; i++)
        {
            ...
            if (takenBlockTarget == fallthroughBlockTarget)
            {
                ...
                if (IsIfThenElsePattern(cfgEdges, currentBlock.Address, takenBlockAddr, fallthroughBlockAddr, continuationBlockAddr))
                {
                    ...
                    Console.WriteLine($"IfThenElseAnalyzer found an if-then-else structure. {CodeEmitVisitor.Instance.Visit(ifThenElse.Condition)}. Then {ifThenElse.ThenBlock.Address:X8}, Else {ifThenElse.ElseBlock.Address:X8}.");
                }
            }
        }
    }
