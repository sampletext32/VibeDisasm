...
public class InstructionViewerPanel : IImGuiPanel
{
    ...
    private void RenderNavigationControls()
    {
        ...
        bool enterPressed = ImGui.InputText("Address", ref _addressInput, 20, 
            ImGuiInputTextFlags.CharsHexadecimal | ImGuiInputTextFlags.EnterReturnsTrue);
        
        ImGui.SameLine();
        bool jumpClicked = ImGui.Button("Jump");
    ...
    {
        if (ulong.TryParse(_addressInput, System.Globalization.NumberStyles.HexNumber, null, out ulong address))
        {
    ...
    private unsafe void RenderInstructionsTable()
    {
        ...
        if (_programmaticSelection && _viewModel.SelectedInstructionIndex >= 0)
        {
            ...
            float scrollPos = ImGui.GetCursorStartPos().Y + 
    ...
    private void RenderInstructionRow(int index)
    {
        ...
        bool isSelected = index == _viewModel.SelectedInstructionIndex;
        ...
        string addressText = $"0x{instruction.Address:X}";
