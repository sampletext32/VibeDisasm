...
public static class ProgramEndpoints
{
    ...
    private static async Task<IResult> ImportProgram(ImportProgramHandler handler, Guid projectId)
    {
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
    private static async Task<IResult> ListProgramsByProject(ListProgramsHandler handler, Guid projectId)
    {
        var result = await handler.Handle(projectId);
        return result.IsSuccess
            ? Results.Ok(result.Value)
